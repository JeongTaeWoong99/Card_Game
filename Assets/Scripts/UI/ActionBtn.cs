...
public class ActionBtn : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Services.Get<CardManager>().IsDamagePreviewActive)
        {
            SetVisible(false);
            return;
        }
        ...
        {
            _btnText.text = "세팅 완료";
            SetActiveLook(Services.Get<BoardPlacement>().IsMyPlaceDone && Services.Get<BoardPlacement>().IsOtherPlaceDone);
        }
        else
        {
            _btnText.text = "턴 종료";
            SetActiveLook(Services.Get<TurnManager>().myTurn && !Services.Get<TurnManager>().isLoading);
        }
    }
    ...
    public void OnClick()
    {
        if (Services.Get<TurnManager>().isLoading) // 로딩 중 입력 무시
        {
            return;
        }
        ...
        if (Services.Get<TurnManager>().IsSetupPhase)
        {
            if (Services.Get<BoardPlacement>().IsMyPlaceDone && Services.Get<BoardPlacement>().IsOtherPlaceDone)
            {
                Services.Get<TurnManager>().OnSetupDone();
            }
            else
            {
                Services.Get<CardManager>().ShowWarning("아직 배치가 끝나지 않았습니다.\n6장의 카드를 모두 배치해 주세요.");
            }
        }
        else if (Services.Get<TurnManager>().myTurn)
        {
