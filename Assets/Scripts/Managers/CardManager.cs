...
public class CardManager : MonoService<CardManager>
{
    ...
    public void CardMouseUp()
    {
        ...
        if (_onMyCardArea)
        {
            Services.Get<BoardPlacement>().RemoveMyEmptyEntity();
        }
        else
        {
    ...
    private void CardDrag()
    {
        ...
        if (!_onMyCardArea)
        {
            ...
            Services.Get<BoardPlacement>().InsertMyEmptyEntity(Utils.MousePos.x, Utils.MousePos.y);
...
int            layer = LayerMask.NameToLayer("MyCardArea");
    ...
    private void SetECardState()
    {
        ...
        else if (Services.Get<TurnManager>().IsSetupPhase && _myCards.Count > 0)
        {
    ...
    public bool TryPutCard(bool isMine, bool isFrontRow)
    {
        ...
        if (Services.Get<BoardPlacement>().SpawnEntity(isMine, card.item, isFrontRow, spawnPos))
        {
    ...
    public void CheatAutoPlaceMyCards()
    {
        ...
        Services.Get<BoardPlacement>().RemoveMyEmptyEntity(); // 드래그 미리보기 정리

        foreach (Card card in new List<Card>(_myCards))
        {
            if (!Services.Get<BoardPlacement>().CheatPlaceMyCard(card.item))
            {
                continue;
            }
    ...
    public void ShowFieldPreview(Entity entity)
    {
        ...
        BoardInputController boardInput = Services.Get<BoardInputController>();
    ...
    public void ShowDamagePreview(Entity attacker, Entity defender)
    {
        ...
        var (dealt, counter) = Services.Get<CombatSystem>().PredictDamage(attacker, defender);
    ...
    public void SkillCardMouseDown(SkillCard card)
    {
        ...
        if (!Services.Get<ManaManager>().CanAfford(true, card.skill.manaCost))
        {
    ...
    private void SkillCardDrag()
    {
        ...
        if (_onMyCardArea)
        {
            ...
            if (isTargeting)
            {
                Services.Get<BoardInputController>().ConsumeSkillTarget(); // 타겟 피커 해제
            }
        ...
        bool hideForTarget = isTargeting && Services.Get<BoardInputController>().PickSkillTarget(Utils.MousePos);
    ...
    public void SkillCardMouseUp(SkillCard card)
    {
        ...
        else if (skill.targeting == ESkillTargeting.None)
        {
            ...
            if (Services.Get<ManaManager>().TrySpend(true, skill.manaCost))
            {
        ...
        else
        {
            ...
            Entity target = Services.Get<BoardInputController>().ConsumeSkillTarget();
            if (target != null && Services.Get<ManaManager>().TrySpend(true, skill.manaCost))
            {
                DiscardSkillCard(true, card);
    ...
    public bool TryCastOtherSkill()
    {
        ...
        var castable = _otherSkillCards.FindAll(card =>
            Services.Get<ManaManager>().CanAfford(false, card.skill.manaCost) &&
            (card.skill.targeting == ESkillTargeting.None || Services.Get<EntityManager>().GetRandomFront(false) != null));
        ...
        Entity target = skill.targeting == ESkillTargeting.MyEntity
            ? Services.Get<EntityManager>().GetRandomFront(false) // 상대 입장의 아군 전방
            : null;

        Services.Get<ManaManager>().TrySpend(false, skill.manaCost);
    ...
    private IEnumerator PlaySkillCo(Skill skill, bool isMine, Entity target)
    {
        ...
        {
            Services.Get<SkillSystem>().Cast(skill, isMine, target);
        ...
        Services.Get<SkillSystem>().Cast(skill, isMine, target); // 카드가 사라진 뒤 효과 적용
    }
